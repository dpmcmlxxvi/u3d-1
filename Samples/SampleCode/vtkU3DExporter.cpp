...
//  vtkActor2D *anTextActor2D, *aPart2D;
    ...
    IFXDEBUG_STARTUP();
    // This is known to cause issues? https://sourceforge.net/p/u3d/discussion/557102/thread/001c3a24/?limit=25#fe53
    result = IFXCOMInitialize();

    if (!IFXSUCCESS(result)) {
        vtkErrorMacro("initialize error.");
        return;
    }

    {
        ...
        if (!IFXSUCCESS(result)) {
            vtkErrorMacro("scene initialize error.");
            return;
        }
